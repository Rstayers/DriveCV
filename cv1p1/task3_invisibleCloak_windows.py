...
while(cap.isOpened()):
    ...
    # this is a mustard yellow/ orange color of my sweater in the image provided
    lower_hsv = np.array([16,210,110])
    upper_hsv = np.array([21,240,255])
    ...
    res2 = cv2.bitwise_and(bgr,bgr,mask=maskInv)
    cv2.imshow("res2", res2)
    ...
    #######################################################
    final_output = cv2.addWeighted(res1,res_scale,res2,1.5,0)
